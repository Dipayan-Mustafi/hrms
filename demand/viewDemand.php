...
function PrintView(m,y,q){
  window.open("printDemand?m="+m+"&y="+y+"&"+q, "Print Demand", "toolbars=No,statusBar=No,width=600, height=400");
}
...
<div class="contDiv">
<div id="detButton"><input type="button" value="Office Copy" onclick="navigate('conDemand?mnth=<?= $mnth?>&yr=<?= $yr?>&q=<?= $qry?>');" accesskey="C"><input type="button" value="Print" onclick="PrintView('<?= $mnth?>', '<?= $yr?>', '<?= $qry?>')" accesskey="P"><input type="button" value="Back" onclick="goBack();" accesskey="B"></div>
...
?>
		<table border="1" style="width:295mm; page-break-after:always; font-family:times; font-size:9pt;" cellpadding="3" cellspacing="0" >
			...
			<tr>
				<th valign="top" align="center" colspan="22">
				<div class="divCell" style="width:30mm;">Business Area </div><div class="divCell" style="width:60mm;"><?= $ofFres[1]?></div>
				<div class="divCell" style="width:30mm;">Unit</div><div class="divCell" style="width:60mm;"><?= $secFres[1]?></div>				</th>
			</tr>		
		<tr>
			...
			while ($lcatFres=$obj->FetchRow($lcatRes)){
			?>
			<th valign="top" align="center">Balance <?= $lcatFres[1]?></th>
			<? }?>
		...
		<tr>
			...
			?>
			<td valign="top" align="center"><?= sprintf("%0.2f", $sumBalance)?><br /><?= $sumInst?></td>
			<? }?>
			<?php 
		...
		<tr>
			...
			while ($lcatFres=$obj->FetchRow($lcatRes)){
				$t= $lcatFres[0];
			?> 
			<th valign="top" align="center"><?= sprintf("%0.2f",$totloan[$t])?><br /><?= sprintf("%0.2f",$totInt[$t])?></th>
			<?
			}
			?>
			<?
