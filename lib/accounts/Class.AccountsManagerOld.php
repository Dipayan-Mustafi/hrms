...
class AccountsManager{
	...
	public function CreateLedger($cid, $led, $ally,$hd=0,$atyp, $ttyp, $fyr){
		...
		if ($insRes){
			...
			$getLastId=$this->obj->lastid("ledgerlist", "ledName='$led' and ledHead='$hd' and acntTyp=$atyp");
			$this->CreateLedgerMaster($led,$ally,$hd,$atyp,$fyr,$cid,$ttyp, $getLastId);
		}
	
	}
	private function CreateLedgerMaster($lname,$alias, $ltyp,$atyp,$fyr, $cid,$ttyp, $listid){
	...
	public function AccountEntry($dt,$vchno, $lastInd, $vTyp, $ttyp, $acntName,$acntTyp, $amount, $remarks="", $fyr, $cID, $subhd="NULL" ){
		
		$ant=$this->getAccountID($cID,$acntName);
		$fld="TransDt, VchNo, VchIndex, VchTyp, TransTyp, LedID, AcntTyp, Amount, remarks, finYear, compID, subhead";
		$val="'$dt', '$vchno', $lastInd, $vTyp, $ttyp, $ant, $acntTyp, $amount, '$remarks', '$fyr', $cID, '$subhd'";
		
		$chk="VchIndex=$lastInd and VchTyp=$vTyp and TransTyp=$ttyp and LedID=$ant and Amount=$amount and compID=$cID";
	...
	private function getAccountID($cid, $acntName){
	
		$res=$this->obj->select("ledgerlist", "ledName='$acntName'");
		$fres=$this->obj->fetchrow($res);
		
		$mstRes=$this->obj->Select("ledgermaster","ledListID=$fres[0] and compID=$cid");
