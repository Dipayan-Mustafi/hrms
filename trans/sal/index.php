...
if ($_REQUEST['bshow']){
		...
		//print $set->JScriptAlert("$chkRows and $usrFres[9]");
		if($chkRows > 0 && $usrFres[9] > 2){
			print $set->JScriptAlert("Salary Already Generated. No Updation Access for $usrFres[1]");
			$set->redirect("index");
		}else{
	...
	<div class="displayBox" align="center" style="width:96%; box-shadow:0.5em 0.5em 0.5em #CCCCCC">
			
			<form name="form2" method="post" action="salSet.php" style="border-radius:6px; border:solid 1px #a1a1a1; background-color:#E2E2E2; padding:8px;"/>
			
			<input type="hidden" name="mnth" id="mnth" value="<?= $m?>"/>
			<input type="hidden" name="yr" id="yr" value="<?= $yr?>"/>
			<input type="hidden" name="dept" value="<?= $dept?>" />
			<input type="hidden"  name="totd" id="totd" value="<?= $d?>" /> 
			<input type="hidden" name="sday" id="sday" value="<?= $hday?>" />
			<input type="hidden" name="hday" id="hday" value="<?= $hdSum?>" />
			<input type="hidden" id="twd" value="<?= $actDays?>"/>	
			<table width="100%" border="0" cellpadding="3" cellspacing="0" bgcolor="#FFFFFF" id="listTbl">
			  <tr style="width:96%; border-radius:6px; border:solid 1px #a1a1a1; background-color:#999999; padding:8px; list-style-position:inside;">
			  	<th width="4%" align="center"   valign="top">Sl No</th>
				<th width="19%" align="center"   valign="top">Employee</th>
				<th width="10%" align="center"   valign="top">Designation</th>
				<th width="7%" align="center"   valign="top">Total Days In the Month </th>
				<th width="7%" align="center"   valign="top">Sundays</th>
				<th width="8%" align="center"   valign="top">Holidays</th>
				<th width="12%" align="center"   valign="top">Absent </th>
				<th width="9%" align="center"   valign="top">Present </th>
				<th width="5%" align="center"   valign="top">Late</th>
				<th width="8%" align="center"   valign="top">Leave</th>
				<th width="5%" align="center"   valign="top">Salary days </th>
				<th width="6%" align="center"   valign="top">&nbsp;</th>
		      </tr>
			  <?
			 
			$strDt="$yr-$m-01";	
			  $res=$obj->select("empmaster", "(empTyp=1 or empTyp=3) and empDept='$dept' and empDoj <'$ldt' and (modDate>'$strDt' or modDate='0000-00-00') order by empName");
			  ...
			  while ($fres=$obj->fetchrow($res)){
					...
					//print "<input name='twd[]' type='hidden' id='twd<?= $r' value='$actDays' />";
					...
					$levRes=$obj->sumfield("emplevconfig", "qty", "empCode='$fres[2]' and finYear='$fyr'");
			  ...
			  <tr style="border-radius:6px; border:solid 1px #a1a1a1; background-color:#E2E2E2; padding:8px;">
				<td align="center" valign="top" style="border-right:hidden; border-left:hidden; border-top:hidden; border-bottom:1px solid border-bottom:1px solid"><?= $r?></td>
				<td align="left" valign="top" style="border-right:hidden; border-left:hidden; border-top:hidden; border-bottom:1px solid border-bottom:1px solid"><div class="divCell" style="width:70%;border-right:hidden; border-left:hidden; border-top:hidden; border-bottom:1px solid border-bottom:1px solid"><?= $fres[3]?></div>
			     <div class="divCell" align="right"style="border-right:hidden; border-left:hidden; border-top:hidden; border-bottom:1px solid border-bottom:1px solid"><input name="ecode[]" style="text-align:center" type="text" size="3" id="ecode<?= $r?>" readonly="readolny" value="<?= $fres[2]?>" /></div></td>
				<td align="center" valign="top"  ><?= $dsgFres[1]?></td>
				<td align="center" valign="top"  ><?= $d?></td>
				<td align="center" valign="top"  ><?=$hday?></td>
				<td align="center" valign="top"  ><?=$hdSum?></td>
				<td align="center" valign="top"  ><input type="text" style="text-align:center" name="abs[]" id="abs<?=$r?>" size="5" value="<?=$absent?>" onblur="absCal(<?= $r?>);"/>			    </td>
				<td align="center" valign="top"  ><input name="prsnt[]" type="text" id="prsnt<?= $r?>" size="5" value="<?= $prsntDay?>" onblur="calLev(<?= $r?>);" />
			    </td>
				<td align="center" valign="top"  ><input name="lat[]" type="text" id="lat<?= $r?>" size="5" onblur="calLate(<?=$r?>, <?=$availLeave?>)" value="<?= $attnLate?>" /></td>
				<td align="center" valign="top"  ><input name="leave[]" type="text" id="leave<?= $r?>" size="5" value="" /></td>
			  ...
			  ?>
			   <tr>
			    <td colspan="12" align="Center" valign="top" style="background:#B08377">
				<input name="bsav" type="submit" style="min-width:96%; font-weight:800;" id="bsav" value="Proceed >>" /></td>
		       </tr>
		</table>
